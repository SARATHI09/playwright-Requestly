...
export class CustodianPages {
	...
	async addAttachment() {
		...
		const field = await this.page.getByRole('button', { name: this.selectors.attachmentButton }).first();
	...
	async moveToQuery() {
		...
		const field = this.page.getByPlaceholder(this.selectors.queryField);

		if (await field.isVisible() && await field.isEnabled()) {
			await field.fill(custodianData.projectName);
		}
